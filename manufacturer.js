...
router.post('/create',
    check("id", "Must be 4 numbers").notEmpty().isInt().isLength({ min: 4, max: 4 }),
    ...
    function (req, res, next) {
        ...
        if (!errors.isEmpty()) {
            ...
            var promise = connection.raw(
                ...
                values (?,?,?)
                `,
             [sanitizeHtml(req.body["id"]),sanitizeHtml(req.body["name"]), sanitizeHtml(req.body["image_url"])]
            )
        };
        promise.then(function (result) {
            res.json(201,{
...
router.put('/update/:id', 
    ...
    promise = connection.raw(
        ...
        where id = ?
        `,
        [sanitizeHtml(req.body["name"]), req.params["id"]]
    )
